...
class TestChatConsumers(TransactionTestCase):
    ...
    async def test_connect_authenticated_not_participant(self):
    ...
    async def test_participant_send_and_receive(self):
        ...
        self.assertEqual(data.get('conversation_id'), conv.id)
        self.assertEqual(data.get('content'), 'hola desde prueba')
