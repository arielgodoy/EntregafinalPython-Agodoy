...
- **Authorization**: `Tareas` + `ingresar` para leer el feed; `modificar` para crear/editar/vincular/desvincular; `supervisor` (S de VICMEAS) para ocultar/restaurar. La lectura del feed requiere usuario activo y Empresa activa, pero no participación funcional; las mutaciones, el reconocimiento y el seguimiento requieren participación funcional efectiva (`Tarea.responsable OR TareaParticipante OR Hito.responsable` para Hitos de la Tarea con `anulado=False`). El creador no tiene bypass y no se crean permisos nuevos.
