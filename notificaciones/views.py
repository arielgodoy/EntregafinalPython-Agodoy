...
class NotificacionesTopbarView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Topbar"
...
class NotificacionesMarkReadView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Marcar como leída"
    permiso_requerido = "ingresar"
    ...
    def _handle(self, request, notification_id):
        empresa_id = _get_empresa_id(request)
    ...
    def post(self, request, notification_id, *args, **kwargs):
        return self._handle(request, notification_id)

    def get(self, request, notification_id, *args, **kwargs):
        return self._handle(request, notification_id)


class NotificacionesMarkAllReadView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Marcar todas como leídas"
    permiso_requerido = "ingresar"
...
class MisNotificacionesView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Mis Notificaciones"
    permiso_requerido = "ingresar"
...
class VerNotificacionView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Ver Notificación"
    permiso_requerido = "ingresar"
...
class CentroAlertasView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Centro de Alertas"
    permiso_requerido = "ingresar"
...
class ForzarNotificacionesView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Forzar Notificaciones"
    permiso_requerido = "ingresar"
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return VerificarPermisoMixin.dispatch(self, request, *args, **kwargs)
...
class AlertaPersonalizadaView(VerificarPermisoMixin, LoginRequiredMixin, View):
    vista_nombre = "Notificaciones - Alerta Personalizada"
    permiso_requerido = "ingresar"
    ...
    def _get_empresa_objetivo_get(self, request, empresas):
    ...
    def _build_destinatarios(self, request, empresa_objetivo, user_model):
        if not empresa_objetivo:
            return []
        destinatarios_qs = user_model.objects.filter(
            permiso__empresa_id=empresa_objetivo.id,
            is_active=True,
        ).distinct().order_by("username")
    ...
    def get(self, request, *args, **kwargs):
        ...
        empresa_objetivo = self._get_empresa_objetivo_get(request, empresas)
        destinatarios = self._build_destinatarios(request, empresa_objetivo, UserModel)
        context = {
            "empresas": empresas,
            "empresa_objetivo": empresa_objetivo,
            "empresa_objetivo_id": empresa_objetivo.id if empresa_objetivo else "",
