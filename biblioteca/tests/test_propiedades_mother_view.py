...
class PropiedadesMotherViewTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="propiedades-test")
        self.empresa_activa = Empresa.objects.create(codigo="21", descripcion="Activa")
        self.otra_empresa = Empresa.objects.create(codigo="22", descripcion="Otra")

    def test_active_endpoints_share_mother_view_and_keep_action_permissions(self):
        expected = {
            "listar_propiedades": "ingresar",
            "detalle_propiedad": "ingresar",
            "crear_propiedad": "crear",
            "modificar_propiedad": "modificar",
            "eliminar_propiedad": "eliminar",
        }
        found = []
        ...
        self.assertEqual(sum(name == "crear_propiedad" for name, _, _ in found), 2)
        self.assertEqual({vista_nombre for _, vista_nombre, _ in found}, {VISTA_PROPIEDADES})
        self.assertEqual(
